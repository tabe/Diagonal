 ...
 * `base'.
 */
DIAG_FUNCTION uint32_t diag_hash32_rabin_karp(const uint8_t *data, diag_size_t size,
				       uint32_t base);
/*
 * Return the 64-bit Rabin-Karp hash value of `data' of length `size' with
 * `base'.
 */
DIAG_FUNCTION uint64_t diag_hash64_rabin_karp(const uint8_t *data, diag_size_t size,
				       uint64_t base);
 ...
 */
DIAG_FUNCTION uint32_t diag_hash32_adler32(const uint8_t *data, diag_size_t size);
#endif

...
diag_vector_new(diag_size_t length)
{
	...
	s = sizeof(diag_vector_t) + sizeof(diag_vector_element_t) * (size_t)length;
...
void
diag_vector_set(diag_vector_t *v, diag_size_t k, diag_vector_element_t e)
{
	assert(v && e);
	if (k >= v->length) {
		diag_error("exceed vector length %ld: %ld", v->length, k);
...
void
diag_vector_fill(diag_vector_t *v, diag_vector_element_t fill)
{
	diag_size_t k;

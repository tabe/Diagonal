...
int main()
{
	...
	w = diag_vector_copy_from(v, 1);
	ASSERT_EQ_SIZE(2, diag_vector_length(w));
	ASSERT_EQ_CHAR('b', diag_vector_ref(w, 0));
	ASSERT_EQ_CHAR('c', diag_vector_ref(w, 1));
	diag_vector_destroy(w);

	w = diag_vector_copy_from(v, 2);
	ASSERT_EQ_SIZE(1, diag_vector_length(w));
	ASSERT_EQ_CHAR('c', diag_vector_ref(w, 0));
	diag_vector_destroy(w);

	w = diag_vector_copy_from(v, 3);
	ASSERT_EQ_SIZE(0, diag_vector_length(w));
	diag_vector_destroy(w);
